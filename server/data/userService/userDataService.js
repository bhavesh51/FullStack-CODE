...
router.post("/api/user/setUser", function (req, res) {
    var query = "[SetUser] '" + req.body.Name + "', '" + req.body.Email + "', '" + req.body.Phone + "', '" + req.body.Password + "'";
    dbService.executeQuery(query, function (data, err) {
        if (err) {
            throw err;
        } else {
...
router.put("/api/user/putUser", function (req, res) {
    var query = "[PutUser] " + parseInt(req.body.Id) + ", '" + req.body.Name + "','" + req.body.Email + "', '" + req.body.Phone + "', '" + req.body.Password + "'";
    dbService.executeQuery(query, function (data, err) {
        if (err) {
            throw err;
        } else {
